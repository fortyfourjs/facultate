...
int main()
{
    ...
    //algoritm cautare in adancime limitata
    ...
    while(gasit == 0 && nr_noduri != 0)//Cat timp solutie negasita si noduri ≠ vida executa
        ...
        for(int i = 0; i < nr_noduri; i++)//Eliminam primul element din noduri
            noduri[i] = noduri[i + 1];
        nr_noduri--;
        ...
        else
        {
            for(int i = 0; i < n; i++)// Adaugam la final in noduri orasele nevizitate care sunt conectate de nod
            {
                if((a[nod][i] == 1) && ((vizitat[i] == 0) || (vizitat[i] == 1 && adancime[i] > adancime[nod] + 1)))
                    for(i = nr_noduri; i > 0; i--)// Adaugam la inceput in noduri orasele nevizitate care sunt conectate de nod
                        noduri[i] = noduri[i - 1];
                    noduri[0] = i;
                    nr_noduri++;
